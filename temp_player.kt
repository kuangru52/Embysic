...
class PlayerDialogFragment : BottomSheetDialogFragment() {
    ...
    private fun updateMetadata(mediaItem: MediaItem?) {
        ...
        mediaItem.mediaMetadata.let {
            ...
            if (extras != null) {
                ...
                val codec = extras.getString("codec", "鏈煡")
                val bitrate = extras.getInt("bitrate", 0) / 1000
                tvAudioQuality.text = if (isTranscoding) "姝ｅ湪杞爜: $codec ${bitrate}kbps" else "$codec ${bitrate}kbps"
    ...
    private fun toggleFavorite() {
        ...
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(context, if (isFavorite) "宸插彇娑堟敹钘? else "宸叉坊鍔犳敹钘?, Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
                Toast.makeText(context, "鍚屾澶辫触", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun searchNeteaseCover(itemId: String, title: String?, artist: String?) {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val query = if (artist.isNullOrBlank() || artist == "鏈煡姝屾墜") cleanedTitle else "$cleanedTitle $artist"
            ...
            try {
                ...
                // 2. 浣跨敤鏂扮増 v3 璇︽儏鎺ュ彛鑾峰彇灏侀潰
                // 杩欓噷鐨?ids 鍙傛暟闇€瑕佹牸寮忓寲涓?[{"id":songId}]
                ...
                if (!picUrl.isNullOrEmpty()) {
                    withContext(Dispatchers.Main) {
                        if (player?.currentMediaItem?.mediaId == itemId) {
                            // 3. 灏嗘媺鍙栧埌鐨勫皝闈繚瀛樺埌鏈湴鎸佷箙鍖栫紦瀛橈紝渚涘垪琛ㄤ娇鐢?                            saveNeteaseCoverToCache(itemId, picUrl)
                            
                            // 4. 鏇存柊褰撳墠鎾斁鍣ㄧ殑 MediaItem锛岀‘淇濋€氱煡鏍忓悓姝?                            updateMediaItemArtwork(picUrl)
    ...
    private fun updateMediaItemArtwork(url: String) {
        ...
        // 娉ㄦ剰锛歁edia3 涓嶆敮鎸佺洿鎺ュ湪鎾斁鏃舵浛鎹㈠崟涓?Item 鐨?Metadata 涓斾繚鎸佽繘搴?        // 浣嗘垜浠彲浠ラ€氳繃 replaceMediaItem 鏉ュ疄鐜?        val currentIndex = p.currentMediaItemIndex
    ...
    private fun showLyrics() {
        ...
        if (lyricsCache.containsKey(itemId)) {
            ...
            lyricsAdapter.lines = listOf(LrcLine(0, "姝ｅ湪鍔犺浇姝岃瘝..."))
        ...
        lifecycleScope.launch {
            try {
                ...
                if (rawLines.isNullOrEmpty()) {
                    ...
                    val metadata = mutableListOf(LrcLine(-1, title ?: "鏈煡鏇插悕"), LrcLine(-1, artist ?: "鏈煡姝屾墜"))
    ...
    private fun searchNeteaseLyrics(itemId: String, title: String?, artist: String?) {
        val cleanedTitle = title?.replace(Regex("\\s*[([\\[].*[\\])]]"), "")?.trim() ?: ""
        val isUnknown = artist.isNullOrBlank() || artist.contains("鏈煡") || artist.contains("Unknown")
        ...
        lifecycleScope.launch {
            try {
                ...
                if (!lrcText.isNullOrBlank()) {
                    val metadata = mutableListOf(LrcLine(-1, title ?: "鏈煡鏇插悕"), LrcLine(-1, artist ?: "鏈煡姝屾墜"), LrcLine(-1, "鏉ユ簮: 缃戞槗浜戦煶涔?))
    ...
    private fun showNoLyrics(itemId: String, title: String?, artist: String?) {
        val finalLines = listOf(LrcLine(-1, title ?: "鏈煡鏇插悕"), LrcLine(-1, artist ?: "鏈煡姝屾墜"), LrcLine(0, "鏈壘鍒板湪绾挎瓕璇?))
        lyricsCache[itemId] = finalLines
        if (rvLyrics.isVisible && player?.currentMediaItem?.mediaId == itemId) {
            lyricsAdapter.lines = finalLines
        }
